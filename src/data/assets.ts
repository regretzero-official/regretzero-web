...
export const MONTHS_REGRET = (CURRENT_YEAR - START_YEAR) * 12;
export const MONTHS_FUTURE = (FUTURE_END_YEAR - CURRENT_YEAR) * 12;
export const TOTAL_MONTHS = MONTHS_REGRET + MONTHS_FUTURE;
export const AVG_BANK_RATE = 0.025;
...
export function buildAsset(item: any): Asset {
  ...
  if (ticker === "005930.KS" || ticker === "삼성전자") {
    gMin = 1.5;
    gMax = 2.2;
  }
...
export const DEFAULT_ASSET =
  ASSETS.find((asset) => asset.ticker === "NVDA" || asset.ticker === "AAPL") ?? ASSETS[0];
...
export function generateTimeSeries(asset: Asset, mode: "REGRET" | "SINGULARITY"): ChartPoint[] {
  ...
  if (asset.ticker === "NVDA" || asset.ticker === "TSLA") volatility = 0.2;
  ...
  const shockSum = shocksRegret.reduce((acc, current) => acc + current, 0);
  ...
  for (let month = 1; month <= MONTHS_REGRET; month += 1) {
    ...
    let convictionMessage: string | undefined;
    ...
    if (newAssetBase > runningMax) {
      ...
      convictionMessage =
        month % 2 === 0
          ? "시장의 공포가 커질수록 좋은 자산을 싸게 담을 기회가 열립니다."
          : "가격보다 기업의 본질을 보세요. 이 구간은 흔들림이 아니라 축적의 시간입니다.";
    }
  ...
  if (mode === "SINGULARITY") {
    let annualGrowthRate = 0.05;
    if (asset.group === "가상자산") annualGrowthRate = 0.4;
    else if (["TSLA", "NVDA", "MSFT", "AAPL", "GOOGL"].includes(asset.ticker)) annualGrowthRate = 0.25;
    ...
    for (let month = 1; month <= MONTHS_FUTURE; month += 1) {
      ...
      if (newAssetBase > runningMax) {
        ...
        inDrawdown = true;
        isConvictionPoint = true;
        convictionMessage = "기술 혁신의 큰 파도는 변동성을 동반합니다. 장기 관점이 가장 강한 무기입니다.";
      }
