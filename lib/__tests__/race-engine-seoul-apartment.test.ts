...
describe("race-engine seoul apartment proxy integration", () => {
  it("builds a proxy price series for seoul apartment", () => {
    const series = buildAssetPriceSeries("seoul_apt", bundle, "2015-08-01", "2025-08-01");

    expect(series[0]).toEqual({ date: "2015-08-01", price: 520_000_000 });
    expect(series[series.length - 1]).toEqual({ date: "2025-08-01", price: 1_430_000_000 });
  });

  it("supports seoul apartment in race data alongside market assets", () => {
    ...
    expect(Number(race.points[race.points.length - 1]?.seoul_apt ?? 0)).toBeCloseTo(
      10_000_000 * (1_430_000_000 / 520_000_000),
      -1,
    );
  });

  it("supports start-point calculations for seoul apartment", () => {
