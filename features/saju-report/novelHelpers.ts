...
export function novelDepth(
  ...
  const flavorCounsel =
    flavor === "heart"
      ? `속마음 상담으로 한 스푼 더: ${p}가 너를 떠올리는 순간이 있어도, 그걸 ‘지금 열어달라는 신호’로 읽지 마. 살짝 스치는 그리움은 **마음엔 남아 있는데 손대긴 무서운** 쪽에 가깝고, 확인을 강요하면 문이 닫혀. ${beat}에서 네가 고를 표정은 ‘간절’이 아니라 **편함**이야.`
      : flavor === "breakup"
...
${p}와의 이별(${breakup}, ${months})을 떠올릴 때, 너는 두 갈래로 갈려. 한쪽은 “아직 가능할까”이고, 다른 쪽은 “내가 또 매달리는 건 아닐까”야. 둘 다 정상이야. ${narrator}${topicParticle(narrator)} 그 둘을 싸우게 두지 않아. **여기서 멈춰. 매달림은 문을 닫아.**
...
타이밍도 달력보다 상태야. “오늘이 며칠이라서”가 아니라 “오늘 내가 정돈됐나”가 먼저야. 상태가 흐리면 좋은 타이밍도 독이 되고, 상태가 맑으면 평범한 날도 시험 운전이 돼.
