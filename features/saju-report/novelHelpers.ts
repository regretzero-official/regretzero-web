...
export function novelDepth(
  ...
  sceneHook: string,
  narrator: string,
  flavor: "heart" | "breakup" | "strategy" | "reason" = "heart",
): string {
  const dm = dayMasterLabel(chart.dayMaster, chart.dayMasterElement);
  ...
  const flavorLead =
    flavor === "heart"
      ? `${p}의 속마음은 확인 강요가 아니라 살짝 스치는 그리움이야. 스치고, 멈추고, 닫아. 네가 그 그리움을 ‘무정’으로 읽으면 장문이 나오고, 장문이 나오면 상대는 더 손대기 무서워져. **지금은 온기보다 안전감이다.** 여기서 멈춰.`
      : flavor === "breakup"
        ? `남겨둘지 놓을지의 핵은 사랑 점수가 아니야. **네가 먼저 괜찮은 상태가 돼야 해.** ${p}를 악역으로 두어도, 성인으로 두어도—결정은 네가 작아지지 않는 쪽으로. 원국을 봐도 ‘퍼주는 결’이 과하면 소모가 먼저 와.`
        : flavor === "reason"
          ? `진짜 이유는 표면 말과 속 피로를 나눠 읽어야 해. **사랑이 없어서가 아니라 지쳐서인 결**이 많아. 제3자 가능성은 단정이 아니라 결로. 진실을 알아도 전송으로 증명할 필요는 없어. **지금은 네 하루가 먼저다.**`
          : `전략의 지표는 답장 속도가 아니야. **네가 무너지지 않는 하루**야. ${p}에게 보내는 한 줄보다, 보내고 나서의 네 일정이 더 중요해. **설득보다 일상이 먼저다.**`;

  const flavorChoice =
    flavor === "heart"
      ? `가벼운 온기(타이밍·상태 될 때) / 지금 당장 다시 만나자고 밀지 않기 / 네가 안전해 보이는 루틴`
      : flavor === "breakup"
        ? `남겨둘 조건·놓을 조건을 문장으로 적기 / 자존 루틴 지키기 / 감정 최고점에서 결정 미루기`
        : flavor === "reason"
          ? `표면·속을 메모로만 정리 / 제3자 추궁 금지 / 생활 리듬 회복 / 가벼운 안부(괜찮은 날만)`
          : `생활 리듬 회복 / 메모장에만 초안 / 가벼운 안부(괜찮은 날만) / 거절·읽씹 후에도 하루 굴리기`;

  const flavorCounsel =
    flavor === "heart"
      ? `속마음 상담으로 한 스푼 더: ${p}가 너를 떠올리는 순간이 있어도, 그걸 ‘지금 열어달라는 신호’로 읽지 마. 살짝 스치는 그리움은 **마음엔 남아 있는데 손대긴 무서운** 쪽에 가깝고, 확인을 강요하면 거리가 멀어져. ${beat}에서 네가 고를 표정은 ‘간절’이 아니라 **편함**이야.`
      : flavor === "breakup"
        ...
        : flavor === "reason"
          ? `진짜 이유 상담으로 한 스푼 더: “다른 사람이었을까”와 “내가 부족했을까”를 한 시험지로 합치지 마. 표면 말은 입구고, 속은 피로·페이스인 경우가 많아. ${beat}에서 네가 고를 건 추궁이 아니라 **이해한 뒤에도 네 하루를 지키는 선택**이야.`
          : `전략 상담으로 한 스푼 더: 말해도 될 타이밍이 보여도 ‘허가증’이 아니야. 시험 운전에 가까워. ${beat}에서 성공은 답장이 아니라, **보내고 나서도 네가 일정대로 사는가**야. 그게 안정으로 읽혀.`;
...
${p}와의 이별(${breakup}, ${months})을 떠올릴 때, 너는 두 갈래로 갈려. 한쪽은 “아직 가능할까”이고, 다른 쪽은 “내가 또 매달리는 건 아닐까”야. 둘 다 정상이야. ${narrator}${topicParticle(narrator)} 그 둘을 싸우게 두지 않아. **여기서 멈춰. 매달리면 더 멀어져.**
...
지쳐 헤어진 사람은 그리움과 두려움이 한몸이야. 네가 안전해 보일수록 그리움이 말하고, 네가 추궁으로 보일수록 두려움이 말해. 헤어진 지 ${months}, “${breakup}” 결이면 ${p}는 **미움이 아니라 피로·자기보호**다. **이게 맞아.**

타이밍도 달력보다 상태야. “오늘이 며칠이라서”가 아니라 “오늘 내가 좀 괜찮은가”가 먼저야. 상태가 흐리면 좋은 타이밍도 부담이 되고, 상태가 맑으면 평범한 날도 시험 운전이 돼.
...
${narrator}${subjectParticle(narrator)} 분명히 말해. “지금은 이렇다—**네가 무너지지 않는 기술**이 먼저다. 인연의 다음 흐름은 그 기술 위에 올라온다. 기술이 먼저야.”

**흔들리지 마. 밤은 길어도, 네 선택은 짧고 단호하게.** 헤어진 지 ${months}. “${breakup}”. 그 숫자를 달력 저당으로 쓰지 마. **상태 우선.** 상태가 괜찮은 날이, 선택해도 되는 날이야.`;
}
...
export function applyPads(
  ...
  chart: SajuChart,
  narrator: string,
  flavor: "heart" | "breakup" | "strategy" | "reason",
) {
...
export function sceneBlocks(
  ...
  const beats = ["남은 마음", "온도", "연락", "자존", "전략", "침묵", "잔향", "선택", "루틴", "문", "온기", "중심"];
  const parts: string[] = [
    `${y}. ${voiceName}${topicParticle(voiceName)} 바로 말해줄게. ${p} 생각이 밤에만 커지는 건 약함이 아니라, 깊게 남는 결이야. 다만 그 결을 전송으로 쓰면 부담이 돼.`,
    ...
    `*(근거 한 줄)* 원국·이별·침묵의 결을 겹치면 흐름이 선명해. **지금은 이렇다—네 생활부터, 들이대기는 나중.**`,
...
${beat} 앞에서 네가 흔들릴 때마다, “지금 내가 좀 괜찮은가?”만 물어. YES면 짧은 선택, NO면 전부 내일. 이 질문이 ${beat} 점사의 척추야.

${p}에게 확인하고 싶은 밤일수록, 확인하면 더 안 좋아져. 확인 대신 루틴. 루틴이 쌓이면 말하기가 쉬워져. 기회가 그대로여도—너는 작아지지 않아. 그게 이 긴 점사 상담의 본편이야.
