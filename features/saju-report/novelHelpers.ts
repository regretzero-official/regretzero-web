...
export function novelDepth(
  ...
  const flavorLead =
    flavor === "heart"
      ? `${p}의 속마음은 대개 고열이 아니라 저열이야. 스치고, 멈추고, 닫아. 네가 그 저열을 ‘무정’으로 읽으면 장문이 나오고, 장문이 나오면 상대의 보관함은 더 깊어져. 흐름상 지금은 **온기를 키우기보다 안전감을 쌓는 쪽**이 맞아.`
      : flavor === "breakup"
        ? `남겨둘지 놓을지의 핵은 사랑 점수가 아니야. **네 자존이 버티는 구조**야. ${p}를 악역으로 두어도, 성인으로 두어도—결정은 네가 작아지지 않는 쪽으로. 원국을 봐도 ‘퍼주는 결’이 과하면 소모가 먼저 와.`
        : `전략의 지표는 답장 속도가 아니야. **네가 무너지지 않는 하루**야. ${p}에게 보내는 한 줄보다, 보내고 나서의 네 일정이 더 중요해. 흐름상 설득보다 정돈이 먼저야.`;
...
**흐름상 한줄.** ${beat}에서 네가 진짜로 흔들리는 지점은 정보 부족이 아니야. 폰을 여는 손, 대화창을 올렸다 내리는 버릇, 자정에 문장을 늘리는 습관—그 패턴을 먼저 읽어야 다음 선택이 의미가 있어.
...
일간 **${dm}**을 축으로 보면, 너는 깊게 남는 쪽이야. 깊게 남는 사람은 이별 후에도 해석이 선명해. 선명함을 ‘즉시 행동’으로 옮기지 마. 선명함은 일기에 두고, 행동은 낮의 짧은 언어로만. 이 분리만 돼도 ${beat}의 운이 달라져.

### ${p} 심리 · 타이밍 · 감정 해석

${flavorCounsel}

지쳐 헤어진 사람은 그리움과 두려움이 한몸이야. 네가 안전해 보일수록 그리움이 말하고, 네가 추궁으로 보일수록 두려움이 말해. 헤어진 지 ${months}, “${breakup}” 결이면 ${p} 쪽도 미움보다 피로·자기보호일 가능성이 커. 확정 예언은 없어—**가능성의 지도**로만 읽어.
...
${narrator}${subjectParticle(narrator)} 단호히 말해. “확정 예언은 없어. 있어도 안 줘. 대신 **네가 무너지지 않는 기술**을 길게 줄게. 인연의 다음 흐름은 그 기술 위에 올라오는 보너스에 가깝다. 기술이 먼저야.”
...
원국·흐름상 ${beat}의 핵은 ‘더 잘해주기’가 아니라 **덜 매달리는 안정**이야. ${y}의 진지함은 이미 충분해. 이번 풀이는 진지함을 무게가 아니라 안정으로 바꾸는 연습이야. ${productHooks}

${beat} 앞에서 네가 흔들릴 때마다, “지금 내가 정돈된가?”만 물어. YES면 짧은 선택, NO면 전부 내일. 이 질문이 ${beat} 점사의 척추야.

${p}에게 확인하고 싶은 밤일수록, 확인은 독이 되기 쉬워. 확인 대신 루틴. 루틴이 쌓이면 아이러니하게도 문이 얇아질 여지가 있어. 문이 안 얇아져도—너는 작아지지 않아. 그게 이 긴 점사 상담의 본편이야.

감정 해석 한 줄 더: ${beat}에서 네가 느끼는 조급함은 ‘사랑 부족’ 신호가 아니라 **불확실성을 못 견디는 몸**의 신호일 때가 많아. 몸을 안정시키면 해석이 짧아지고, 해석이 짧아지면 선택이 맑아져.`);
  }
  return parts.join("\n\n");
}
