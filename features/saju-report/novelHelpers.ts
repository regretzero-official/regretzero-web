...
export function novelDepth(
  ...
  const flavorLead =
    flavor === "heart"
      ? `${p}의 속마음은 고열이 아니라 저열이야. 스치고, 멈추고, 닫아. 네가 그 저열을 ‘무정’으로 읽으면 장문이 나오고, 장문이 나오면 상대는 더 손대기 무서워져. **지금은 온기보다 안전감이다.** 여기서 멈춰.`
      : flavor === "breakup"
  ...
  const flavorCounsel =
    flavor === "heart"
      ? `속마음 상담으로 한 스푼 더: ${p}가 너를 떠올리는 순간이 있어도, 그걸 ‘지금 열어달라는 신호’로 읽지 마. 저열의 그리움은 **마음엔 남아 있는데 손대긴 무서운** 쪽에 가깝고, 고열의 확인은 문을 닫게 해. ${beat}에서 네가 고를 표정은 ‘간절’이 아니라 **편함**이야.`
      : flavor === "breakup"
