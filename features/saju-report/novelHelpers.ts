...
${p}와의 이별(${breakup}, ${months})을 떠올릴 때, 너는 대개 두 갈래로 갈려. 한쪽은 “아직 가능할까”이고, 다른 쪽은 “내가 또 매달리는 건 아닐까”야. 둘 다 정상이야. ${narrator}${topicParticle(narrator)} 그 둘을 싸우게 두지 않아. **흐름은 선으로 긋고, 매달림은 차단으로.** 선은 또렷하게, 차단은 단호하게.
...
지쳐 헤어진 사람은 그리움과 두려움이 한몸이야. 네가 안전해 보일수록 그리움이 말하고, 네가 추궁으로 보일수록 두려움이 말해. 헤어진 지 ${months}, “${breakup}” 결이면 ${p} 쪽은 **미움보다 피로·자기보호 쪽**이다. 흐름상 그렇게 단정해.
...
export function sceneBlocks(
  ...
  const parts: string[] = [
    ...
    `헤어진 지 ${months}. 이별 메모는 “${breakup}”. 큰 배신보다 소모·속도 어긋남 쪽이다. 그래서 미련이 남는다. 미움으로 끝났으면 마음이 더 빨리 식었을 거다.`,
...
원국·흐름상 ${beat}의 핵은 ‘더 잘해주기’가 아니라 **덜 매달리는 안정**이야. ${y}의 진지함은 이미 충분해. 이번 풀이 결론—진지함을 무게가 아니라 안정으로 바꿔. ${productHooks}
