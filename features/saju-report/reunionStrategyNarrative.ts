...
${voice.traitClose?.(p) ?? ""}`,
  ...
  const pattern: SajuReportSection = {
    ...
    body: L(
      4,
      `진입→몰입→감정 커짐→차단→잔향. 전략의 적은 3단계(감정 커짐) 반복.
...
**지금은 연락할 때가 아니야. 지금은 다가가지 마. 한두 달에서 세 달쯤 지나야 다시 말할 타이밍이야.** ${months} 기준. 여기서 멈춰.

### ◇ 1개월 — 관망·네 생활부터
적극 재접근 금지다. 에너지는 너에게. 지금은 이렇게 해.

### ◇ 3개월 전후 — 가벼운 안부 타이밍
가벼운 안부. 조건: 매달림 없는 너. **이 구간이 다시 말해도 되는 때다.**
...
세 문장:
1. 지금은 설득하지 마. 가벼운 안부만, 네 생활부터 챙겨. 말은 나중에.
2. 첫 문장은 짧게. 숙제 주지 마.
3. 답장 없어도 네가 괜찮아야 해.
...
${voice.signOff}`,
  ...
  const pads: Record<string, { beat: string; scene: string }> = {
    ...
    strategy: { beat: "3단계", scene: "회수 → 가벼운 안부 → 재판 금지." },
