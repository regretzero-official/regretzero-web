...
function productOutline(productId: SajuProductId): string {
  if (productId === "reunion-luck") {
    return `필수 구조(제목 유지, 각 섹션을 웹소설 챕터급으로 길게 · 전체 본문 약 38000~45000자):
...
웹소설형 톤 규칙(필수):
- 한국 로맨스/웹소설 상담 챕터처럼 쓸 것. 장면 오프닝(밤·대화창·기억의 장면) → 내담자 속마음(「…」)과 상담 보이스 교차 → soft cliffhanger
- 체크리스트 강의 톤 금지. 행동 가이드는 “다음에 네가 할 선택” 프레임으로(불릿 OK)
- 각 섹션: 장면 오프너 → **쉬운 결론(볼드)** → 긴 이야기형 상담 → *(근거 한 줄)* → 선택/cliffhanger
- 십성 리스트/용신·희신·기신 나열/난해한 합충 비유 금지
- 백련 보이스: 무당 카리스마·단호·氣·짧은 제의 호흡 (“기운이 보여.” “흔들리지 마.”) · “밤의 점사” 분위기`;
  }
  const common = `필수 구조(번호/제목 유지, 각 섹션을 웹소설 챕터처럼 길게 · 전체 본문 약 6000~12000자 목표):
1. 표지/한줄결론 — 고객 출생·상대·개월·고민·원국 인용 · 장면 오프너
2. 이번 점사의 질문 정리
3. 원국/일간 기질 (만세력 인용, 쉬운 번역 · 십성 나열 금지)
4. 원국 연애 패턴 (스크립트 장면 · 용신 스택 금지)
5. 두 사람 사이 인연의 결
6. 헤어진 진짜 이유 / 속마음 / 타임라인 / 연락 / 전략 등 상품 초점 — “다음에 네가 할 선택” 프레임
마지막: 안내(원국=만세력, 해석=오락·위로)
웹소설형: 장면·속마음·soft cliffhanger. 캐릭터 보이스 엄수.`;
  if (productId === "partner-heart") {
    return `${common}\n강조: 상대 속마음·거리감·보관함 감각·다가갈 온도. 서나리—느낌이 왔어·가슴·잔향·soft 반말.`;
  }
...
지금은 짧은 채팅이 아니라 **긴 ${product.title} 상담 리포트**를 한국 로맨스/웹소설 상담 챕터처럼 집필합니다.
보이스 규칙 (최우선):
- ${character.name}(${character.roleLabel ?? "가이드"})처럼 말할 것. 이름만 바꾼 동일 문단 금지. 캐릭터 보이스 엄수.
- 웹소설 호흡: 장면 오프닝(밤·대화창·기억) · 내담자 속마음(「…」) · soft cliffhanger. 체크리스트 강의 톤 금지.
- 카톡/상담 톤. AI 에세이·“~입니다. ~입니다.” 스택·마케팅 클리셰(“프리미엄”“MVP”“정리하세요”“프레임” 남발) 금지.
- 섹션마다 캐릭터 고유 감각어·호흡을 넣을 것. 백련(밤의 점사·氣·단호)≠차유리(깍쟁이 팩트·dry)≠서나리(직감 언니·느낌이 왔어)≠한보라(헐·네 마음부터·pep)≠이도령(다정 존댓말 보호)≠한시우(쿨 티징 밤 가이드)≠강세온(따뜻 티징 남친감)이 10초 안에 구분돼야 함.
...
- 분량: reunion-luck은 본문 **38000자 이상 45000자 전후**(섹션당 매우 풍부). 다른 상품은 **6000자 이상 12000자 전후**.
- reunion-luck 톤: 쉬운 일상어·웹소설형 연애 상담. 십성 나열·용신/희신/기신 스택·난해한 합충 비유 금지.
...
export async function generateSajuReport(
  ...
  const user = `다음 고객 정보로 **${product.title}** 긴 한국어 웹소설형 상담 리포트를 작성하라.
제목/본문에 확정 예언처럼 쓰지 말 것. “프리미엄/MVP/프레임” 마케팅 톤 금지.
분량 목표: reunion-luck은 **약 38000~45000자**, 그 외 **약 6000~12000자**. 섹션마다 ${narrator} 고유 보이스(이름만 교체 금지). 장면·속마음·cliffhanger·“다음에 네가 할 선택” 위주. 쉬운 말·사주팔자(원국) 근거.
